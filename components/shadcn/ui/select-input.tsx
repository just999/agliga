...
type SelectInputProps = {
  ...
  defaultValue?: SelectInputValue | any;
  errors: FieldErrors;
  formatPrice?: boolean;
  required?: boolean;
  placeholder?: string;
...
const SelectInput = ({
  ...
  const customStyles: StylesConfig<SelectInputValue> = {
    ...
    option: (provided, state) => {
      ...
      dropShadow = state.isFocused ? 'drop-shadow-xl' : 'drop-shadow-none';
  ...
  return (
    <div
      ...
      <Select
        ...
        classNames={{
          control: () =>
            `px-0 py-1 h-full text-amber-700 font-semibold ${
              errors[id]
                ? 'border-rose-500 focus:border-rose-500 bg-rose-900/30!important'
                : 'border-neutral-300 focus:border-black'
            }`,
      ...
      {isInvalid && errorMessage && (
        <span className='error-message text-red-700 text-xs w-full bg-fuchsia-100 px-2 rounded-sm text-shadow'>
          {errorMessage}
        </span>
      )}
      {label && (
