...
const MemberWDForm = ({ user }: WDFormProps) => {
  ...
  const methods = useForm<WdSchema>({
  ...
  const onSubmit = async (data: WdSchema) => {
    const wdData = JSON.parse(JSON.stringify(getValues()));
    const res = await createWd(wdData);
  };
  return (
    <Card className='w-3/4 mx-auto p-4 shadow-lg'>
      <div className='flex flex-col space-y-4'>
        <HeadingLogo title='Wd' subtitle='' />
        ...
        <form
          ...
          <InputCustom
            className='h-12'
            placeholder='Nominal wd'
            defaultValue={getValues('wdAmount')}
            {...register('wdAmount')}
            isInvalid={!!errors.wdAmount}
            errorMessage={errors.wdAmount?.message as string}
          />
